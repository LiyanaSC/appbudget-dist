...
const useBudgetStore = defineStore('budget', () => {
    ...
    const showBudgetResults = ref$5(true);
    ...
    const incomes = ref$5(Number(0).toFixed(2));
    //calcul des dépenses à partir des transactions du budget sélectionné
    const outcomes = ref$5(Number(0).toFixed(2));
    //calcul du reste à vivre à partir des revenus et des dépenses du budget sélectionné
    const balance = ref$5(Number(0).toFixed(2));
//----------------------------- Bugdget management functions -----------------------------
    //Innitialisation du model budget avec des valeurs par défaut
    const initializeBudget = (budgetName) => {
        selectedBudget.value = localStorage.getItem(budgetName) ? JSON.parse(localStorage.getItem(budgetName)) : null;
        if (!selectedBudget.value) {
            selectedBudget.value = {
                _id: Date.now(), // en attendant une vraie base de données, on utilise un timestamp comme ID unique
                title: budgetName,
                type: 'classique',
                currency: 'EUR',
            ...
            //calcul des revenus à partir des transactions du budget sélectionné
            incomes.value = selectedBudget.value.transactions
                .filter(transaction => transaction.type === 'incomes')
                .reduce((total, transaction) => total + transaction.amount, 0)
                .toFixed(2);
            //calcul des dépenses à partir des transactions du budget sélectionné
            outcomes.value = selectedBudget.value.transactions
                .filter(transaction => transaction.type === 'outcomes')
                .reduce((total, transaction) => total + transaction.amount, 0)
                .toFixed(2);
            //calcul du reste à vivre à partir des revenus et des dépenses du budget sélectionné
            balance.value = (incomes.value - outcomes.value).toFixed(2);
        }
    };

    //ajouter des transactions (incomes ou outcomes) au budget sélectionné  
    const addTransaction = (transaction, budgetId) => {
        ...
        if (isNaN(transaction.amount)) {
            ...
            selectedBudget.value.transactions.push({
                ...transaction,
                _id: Date.now(),//en attendant une vraie base de données, on utilise un timestamp comme ID unique
                date: new Date().toISOString()
            });
            //Mettre à jour la balance après l'ajout de la transaction
            if (transaction.type === 'incomes') {
                incomes.value = (parseFloat(incomes.value) + parseFloat(transaction.amount)).toFixed(2);
            } else if (transaction.type === 'outcomes') {
                outcomes.value = (parseFloat(outcomes.value) + parseFloat(transaction.amount)).toFixed(2);
            }
            balance.value = (incomes.value - outcomes.value).toFixed(2);
            // Mettre à jour la date de mise à jour du budget
            selectedBudget.value.lastUpdated = new Date().toISOString();
            // Sauvegarder le budget mis à jour dans le localStorage
            localStorage.setItem(selectedBudget.value.title, JSON.stringify(selectedBudget.value));
        ...
        if (selectedBudget.value) {
            ...
            if (index !== -1) {
                ...
                selectedBudget.value.transactions[index] = {
                    ...selectedBudget.value.transactions[index],
                    ...updatedTransaction,
                    lastUpdated: new Date().toISOString() // Mettre à jour la date de mise à jour de la transaction
                };
                // Mettre à jour la balance après la modification de la transaction
                incomes.value = selectedBudget.value.transactions
                    .filter(transaction => transaction.type === 'incomes')
                    .reduce((total, transaction) => total + transaction.amount, 0)
                    .toFixed(2);
    ...
    const deleteTransaction = (transactionId) => {
        if (selectedBudget.value) {
            ...
            // Mettre à jour la balance après la suppression de la transaction
            incomes.value = selectedBudget.value.transactions
                .filter(transaction => transaction.type === 'incomes')
                .reduce((total, transaction) => total + transaction.amount, 0)
                .toFixed(2);
...
const monBudget = 'Mon budget'; // Variable réactive pour stocker le titre du budget
...
// ------------------------- Gestions de liste de budget-------------------------
 const showBudgetResult = () => { // Fonction pour afficher les résultats du budget
  ...
  budgetStore.initializeBudget(monBudget); // Appelle la fonction pour initialiser le budget (peut être utilisée pour charger les données du budget sélectionné)
  ...
  return (_openBlock$4(), _createElementBlock$4("nav", _hoisted_1$3, [
    ...
    _createElementVNode$3("button", {
      ...
      _createElementVNode$3("div", { class: "list-title-content" }, [
        _createElementVNode$3("p", { class: "button-text" }, _toDisplayString$1(monBudget))
      ])
    ]),
...
const sidebar = /*#__PURE__*/_export_sfc(_sfc_main$3, [['__scopeId',"data-v-ac395161"]]);
...
const _hoisted_8$1 = { value: "salaire" };
const _hoisted_9$1 = { value: "freelance" };
const _hoisted_10$1 = { value: "pension" };
const _hoisted_11$1 = { value: "allocations" };
const _hoisted_12$1 = { value: "investissements" };
...
return (_ctx, _cache) => {
  return (_openBlock$3(), _createElementBlock$3("div", _hoisted_1$2, [
    ...
    _withDirectives$1(_createElementVNode$2("select", {
      ...
      _withDirectives$1(_createElementVNode$2("option", _hoisted_8$1, "Salaire", 512), [
        [_vShow, __props.incomes]
      ]),
      _withDirectives$1(_createElementVNode$2("option", _hoisted_9$1, "Freelance", 512), [
        [_vShow, __props.incomes]
      ]),
      _withDirectives$1(_createElementVNode$2("option", _hoisted_10$1, "Pension", 512), [
        [_vShow, __props.incomes]
      ]),
      _withDirectives$1(_createElementVNode$2("option", _hoisted_11$1, "Allocations", 512), [
        [_vShow, __props.incomes]
      ]),
      _withDirectives$1(_createElementVNode$2("option", _hoisted_12$1, "Investissements", 512), [
        [_vShow, __props.incomes]
      ]),
...
const Transactions = /*#__PURE__*/_export_sfc(_sfc_main$2, [['__scopeId',"data-v-e498e546"]]);
...
console.log("test", budgetStore.incomes);
});
...
const addIncome = () => {
    ...
    budgetStore.incomes = (parseFloat(budgetStore.incomes) + parseFloat(amountIncomes.value)).toFixed(2);
...
const addOutcome = () => {
    ...
    // Réinitialiser les champs après l'ajout
    budgetStore.outcomes = (parseFloat(budgetStore.outcomes) + parseFloat(amountOutcomes.value)).toFixed(2);
            ...
            : (_openBlock$2(), _createElementBlock$2("div", _hoisted_6, [
                _createElementVNode$1("div", _hoisted_7, [
                  _createElementVNode$1("div", _hoisted_8, [
                    ...
                    _createElementVNode$1("p", _hoisted_9, _toDisplayString(_unref(budgetStore).balance + ' €'), 1)
                  ]),
                ...
                _createElementVNode$1("div", _hoisted_12, [
                  _createElementVNode$1("div", _hoisted_13, [
                    ...
                    _createElementVNode$1("p", _hoisted_14, _toDisplayString('+ ' + _unref(budgetStore).incomes + ' €'), 1)
                  ]),
                  ...
                  (isOpen.value)
                    ? (_openBlock$2(), _createElementBlock$2("div", _hoisted_16, [
                        _createElementVNode$1("ul", null, [
                          _cache[14] || (_cache[14] = _createStaticVNode("<div class=\"transactions-header\" data-v-a4a8da8a><p class=\"data-header\" data-v-a4a8da8a>Montant</p><p class=\"data-header\" data-v-a4a8da8a>Nom</p><p class=\"data-header\" data-v-a4a8da8a>Catégorie</p><div class=\"data-header-blank\" data-v-a4a8da8a></div></div>", 1)),
                        ...
                        _createElementVNode$1("form", _hoisted_17, [
                          ...
                          _withDirectives(_createElementVNode$1("select", {
                            ...
                            _createStaticVNode("<option value=\"salaire\" data-v-a4a8da8a>Salaire</option><option value=\"freelance\" data-v-a4a8da8a>Freelance</option><option value=\"pension\" data-v-a4a8da8a>Pension</option><option value=\"allocations\" data-v-a4a8da8a>Allocations</option><option value=\"investissements\" data-v-a4a8da8a>Investissements</option><option value=\"autre\" data-v-a4a8da8a>Autre</option>", 6)
                          ]), 512), [
                ...
                _createElementVNode$1("div", _hoisted_19, [
                  _createElementVNode$1("div", _hoisted_20, [
                    ...
                    _createElementVNode$1("p", _hoisted_21, _toDisplayString('- ' + _unref(budgetStore).outcomes + ' €'), 1)
                  ]),
                  _createElementVNode$1("button", {
                    class: "see-more-box",
                  ...
                  (isOpenOutcomes.value)
                    ? (_openBlock$2(), _createElementBlock$2("div", _hoisted_23, [
                        _createElementVNode$1("ul", null, [
                          _cache[18] || (_cache[18] = _createStaticVNode("<div class=\"transactions-header-outcomes\" data-v-a4a8da8a><p class=\"data-header\" data-v-a4a8da8a>Montant</p><p class=\"data-header\" data-v-a4a8da8a>Nom</p><p class=\"data-header\" data-v-a4a8da8a>Catégorie</p><div class=\"data-header-blank\" data-v-a4a8da8a></div></div>", 1)),
                        ...
                        _createElementVNode$1("form", _hoisted_24, [
                          ...
                          _withDirectives(_createElementVNode$1("select", {
                            ...
                            _createStaticVNode("<option value=\"logement\" data-v-a4a8da8a>Logement</option><option value=\"courses\" data-v-a4a8da8a>Courses</option><option value=\"transport\" data-v-a4a8da8a>Transport</option><option value=\"santé\" data-v-a4a8da8a>Santé</option><option value=\"divertissement\" data-v-a4a8da8a>Divertissement</option><option value=\"enfants\" data-v-a4a8da8a>Enfants</option><option value=\"autre\" data-v-a4a8da8a>Autre</option>", 7)
                          ]), 512), [
...
const BudgetResults = /*#__PURE__*/_export_sfc(_sfc_main$1, [['__scopeId',"data-v-a4a8da8a"]]);
